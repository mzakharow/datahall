...
def run():
        ...
        if user:
            ...
            with engine.connect() as conn:
                latest_task = conn.execute(text("""
                    SELECT location_id, activity_id, rack, cable_type_id
                ...
                if latest_task:
                    ...
                    try:
                        last_cable_ids = json.loads(latest_task.cable_type_id or "[]")
                    except Exception:
                        last_cable_ids = []
                    st.session_state.last_cable_ids = last_cable_ids
                else:
                    st.session_state.last_location_id = None
                    st.session_state.last_activity_id = None
                    st.session_state.last_rack = ""
                    st.session_state.last_cable_ids = []
        else:
    ...
    if st.session_state.email_checked:
        ...
        cable_options = {c.name: c.id for c in cable_types}
        ...
        default_cables = [name for name, id_ in cable_options.items()
                          if id_ in st.session_state.get("last_cable_ids", [])]
        ...
        if st.button("Confirm"):
            ...
            if current_email != confirmed_email:
                ...
                cable_ids = [cable_options[name] for name in selected_cables]
                response = {
                    ...
                    "rack": rack_input,
                    "cable_type_id": json.dumps(cable_ids),
                    "timestamp": datetime.now()
                }

                with engine.begin() as conn:
                    conn.execute(text("""
                        INSERT INTO technician_tasks (technician_id, location_id, activity_id, rack, source, cable_type_id, timestamp)
                        VALUES (:technician_id, :location_id, :activity_id, :rack, :technician_id, :cable_type_id, :timestamp)
                    """), response)
